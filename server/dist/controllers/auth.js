...
import { sendErrorResponse } from "../shared/utils/api.js";
import { ErrorType } from "../shared/types/errors.js";
...
const sendAuthSuccessResponse = (res, status, body) => {
    const response = {
        message: "Success",
        success: true,
        body: body
...
export const login = async (req, res) => {
    ...
    try {
        ...
        // const user = await User.findOne({email: email});
        if (!user)
            return sendErrorResponse(res, ErrorType.UserNotFound, 400); //res.status(400).json({mesg: "User does not exist."});
